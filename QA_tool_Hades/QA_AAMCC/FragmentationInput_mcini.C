...
void FragmentationInput_mcini(int flag_dcm, const char* input_path_mcini, const char* input_path_aamcc, string output_dirname, string root_fin_filename, string QA_filename, Int_t files)
{
    ...
    std::cout << " A = "<< A << " Ab = " << Ab << " " << pzA << " " << pzB << " " << SqrtSnn << std::endl; 
...
/*    TH2F* hEcc1_vs_B =                new TH2F ("Ecc1_VS_B", ";#epsilon_{1};B, fm",200,0,1,200,0,20);
    ...
    TH1F* hP =                                  new TH1F("P",";P/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPz =                                 new TH1F("P_z",";Pz/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPTnucl =                             new TH1F("Pt_nucl",";P_{t} for nucleons, GeV/c;counts",100,0,1);
    TH2F* hPT_vs_A =                            new TH2F("Pt_vs_A",";P_{t}, GeV/c;A",100,0,4,A+2,0,A+2);
    TH2F* hPz_vs_A =                            new TH2F("Pz_vs_A",";P_{z}/A, GeV/c;A",1000,0,2*abs(pzA),A+2,0,A+2);
    TH2F* hP_vs_A =                             new TH2F("P_vs_A",";P/A, GeV/c;A",1000,0,2*abs(pzA),A+2,0,A+2);
    TH1F* hPznucl =                             new TH1F("P_z_nucleons",";Pz/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPnucl =                              new TH1F("P_nucleons",";Pz/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPimf =                               new TH1F("P_imf",";P/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPheavy =                             new TH1F("P_heavy",";P/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPzimf =                              new TH1F("P_z_imf",";Pz/A, GeV/c;counts",1000,0,3*abs(pzA));
    TH1F* hPzheavy =                            new TH1F("P_z_heavy",";Pz/A, GeV;counts",1000,0,3*abs(pzA));
    ...
    // canvas->Print(char_output_pdf, "pdf");
    // hFragX_vs_Y_1                           ->Draw("COLZ");
    // canvas->Print(char_output_pdf, "pdf");
    // hFragX_vs_Y_2                           ->Draw("COLZ");
    // canvas->Print(char_output_pdf, "pdf");
    // hFragX_vs_Y_3                           ->Draw("COLZ");
    // canvas->Print(char_output_pdf, "pdf");
    // hFragX_vs_Y_4                           ->Draw("COLZ");
    // canvas->Print(char_output_pdf, "pdf");
