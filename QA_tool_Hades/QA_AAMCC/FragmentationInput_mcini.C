...
void FragmentationInput_mcini(int flag_dcm, const char* input_path_mcini, const char* input_path_aamcc, string output_dirname, string root_fin_filename, string QA_filename, Int_t files)
{
    ...
    if(flag_dcm == 0){
        if(files == 1){
            ...
            for (int i = 1; i <= files; i++) {
                fChain->Add(Form("%s/urqmd_aamcc_%d_mcini.root", input_path_mcini, i));
    ...
    {
        for (int i = 1; i <= files; i++) {fChain->Add(Form("%s/urqmd_aamcc_%d_mcini_.root", input_path_mcini, i));}
    ...
    if(flag_dcm == 1){
        TFile *fIn = new TFile(Form("%s/urqmd_aamcc_1_mcini_.root", input_path_mcini), "read");
    ...
    std::cout << A << " " << pzA << " " << pzB << " " << SqrtSnn << std::endl; 
...
/*    TH2F* hEcc1_vs_B =                new TH2F ("Ecc1_VS_B", ";#epsilon_{1};B, fm",200,0,1,200,0,20);
    ...
    TH1F* hEnergy =                             new TH1F("Energy",";E_{event}, GeV;counts",100,0,SqrtSnn*A/2. + 100);
    TH1F* hP =                                  new TH1F("P",";P/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPz =                                 new TH1F("P_z",";Pz/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPTnucl =                             new TH1F("Pt_nucl",";P_{t} for nucleons, GeV/c;counts",100,0,4);
    TH2F* hPT_vs_A =                            new TH2F("Pt_vs_A",";P_{t}, GeV/c;A",100,0,4,A+2,0,A+2);
    TH2F* hPz_vs_A =                            new TH2F("Pz_vs_A",";P_{z}/A, GeV/c;A",1000,0,2*pzB,A+2,0,A+2);
    TH2F* hP_vs_A =                             new TH2F("P_vs_A",";P/A, GeV/c;A",1000,0,2*pzB,A+2,0,A+2);
    TH1F* hPznucl =                             new TH1F("P_z_nucleons",";Pz/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPnucl =                              new TH1F("P_nucleons",";Pz/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPimf =                               new TH1F("P_imf",";P/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPheavy =                             new TH1F("P_heavy",";P/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPzimf =                              new TH1F("P_z_imf",";Pz/A, GeV/c;counts",1000,0,3*pzB);
    TH1F* hPzheavy =                            new TH1F("P_z_heavy",";Pz/A, GeV;counts",1000,0,3*pzB);
    TH1F* hRapidity =                           new TH1F("Y_nucleon",";Rapidity;counts",500,0,4);
    ...
    TH2F* hEnergyA_vs_ImpactParameter_proj =    new TH2F("EperA_VS_B_proj",";b, fm;E_{frag}/A, GeV",200,0,20,SqrtSnn*100,0,SqrtSnn+5);
    TH2F* hNspect_vs_Espect_proj =              new TH2F("Espect_VS_Nspect_proj",";Nspect;Espect, GeV",A,0,A,SqrtSnn*A/2. + 50,0,SqrtSnn*A/2. + 50);
    TH2F* hNspect_vs_sumZ_proj =                new TH2F("sumZ_VS_Nspect_proj",";sumZ;Nspect",Z + 2,0,Z + 2, A,0,A);
    TH2F* hNspect_vs_Zb2 =                      new TH2F("Zb3_VS_Nspect_proj",";Zb3;Nspect",Z + 2,0,Z + 2, A+1,0,A+1);
    TH2F* hNnucl_vs_Nfrag_proj =                new TH2F("Nnucl_vs_Nfrag_proj",";Nfrag;Nnucl",A+1,0,A+1,2*A,0,2*A);
    ...
    TH2F* hImpactParameter_vs_Energy  =         new TH2F("B_VS_Energy_proj",";b, fm;E_{event} (projectile), GeV",200,0,20,SqrtSnn*A/2. + 50,0,SqrtSnn*A/2. + 50);
        ...
        for (int j=0;j<fNpa; j++)
        {
            ...
            if(fMomentum.Pz() > 0)
            {
                ...
                else if (fParticle->GetPdg()==2212) 
                {
                    ...
                    hPz_vs_A                                ->Fill(fMomentum.Pz(), 1);
                    hP_vs_A                                 ->Fill(fMomentum.P(), 1);
                    ...
                    fEnergy=fEnergy+fMomentum.E();
                    sumZ+=1;
                    //std::cout << " I AM HERE" << sumZ << " "  << std::endl;
                    fNnucl +=1;
                    fNspect+=1;
                }
