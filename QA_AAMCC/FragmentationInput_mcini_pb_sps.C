void FragmentationInput_mcini_pb_sps(char* input_dirname, char* output_dirname, Int_t files, Int_t A, Int_t P_beam)
{
    ...
    //TFile* ReadFile = new TFile("../input/QA_dcmqgsm_sps.root");
    ...
    TH1F* hEnergy =                             new TH1F("Energy",";E_{event}, GeV;counts",100,0,36000);
    TH1F* hP =                                  new TH1F("P",";P/A, GeV;counts",400,0,250);
    TH1F* hPTnucl =                             new TH1F("Pt_nucl",";P_{t}, GeV;counts",100,0,1);
    TH2F* hPT_vs_A =                            new TH2F("Pt_vs_A",";P_{t}, GeV;A",100,0,3,208,0,208);
    TH1F* hPnucl =                              new TH1F("P_z_nucleons",";P/A, GeV;counts",400,0,250);
    TH1F* hPimf =                               new TH1F("P_z_imf",";P/A, GeV;counts",400,0,250);
    TH1F* hPheavy =                             new TH1F("P_z_heavy",";P/A, GeV;counts",400,0,250);
    TH1F* hRapidity =                           new TH1F("Y_nucleon",";Rapidity;counts",200,5,7);
    ...
    TH2F* hNfrag_vs_Energy =                    new TH2F("A_VS_E",";Mass number of fragments;E_{frag}, GeV",200,0,200,150,0,36000);
    ...
    TH2F* hNfrag_vs_Rapidity =                  new TH2F("A_VS_Y",";Mass number of fragments;Rapidity",200,0,200,500,5,7);
    TH2F* hImpactParameter_vs_Energy_Fragment = new TH2F("B_VS_E",";b, fm;E_{event}, GeV",200,0,20,250,0,36000);
    TH2F* hImpactParameter_vs_Rapidity =        new TH2F("B_VS_Y",";b, fm;Rapidity",200,0,20,250,5,7);
    TH2F* hRapidity_vs_Energy =                 new TH2F("Y_VS_E",";E, GeV;Rapidity",360,0,36000,250,5,7);
    ...
    TH2F* hEnergyA_vs_ImpactParameter_proj =          new TH2F("EperA_VS_B_proj",";b, fm;E_{frag}/A, GeV",200,0,20,250,0,250);
    //TH2F* hEnergyA_vs_ImpactParameter_targ =          new TH2F("EperA_VS_B_targ",";b, fm;E_{frag}/A, GeV",200,0,20,200,0,4);
    TH2F* hNspect_vs_Espect_proj =          new TH2F("Espect_VS_Nspect_proj",";Nspect;Espect, GeV",100,0,100,150,0,36000);
    ...
    TH2F* hEnergyE_vs_sumZ_proj =          new TH2F("EnergyE_VS_sumZ_proj",";sumZ;E_{event}, GeV",83,0,83,175,0,36000);
    TH2F* hEnergyE_vs_Nnucl_proj =          new TH2F("EnergyE_VS_Nnucl_proj",";Nnucl;E_{event}, GeV",90,0,90,175,0,36000);
    TH2F* hEnergyE_vs_Nimf_proj =          new TH2F("EnergyE_VS_Nimf_proj",";N_{IMF};E_{event}, GeV",15,0,15,35,0,36000);

    //TH2F* hEnergyProj_vs_EnergyTarg        =          new TH2F("E_proj_VS_E_targ",";E_{event} (target), GeV;E_{event} (projectile), GeV",150,0,300,150,0,36000);
    //TH2F* hImpactParameter_vs_Energy_targ  =       new TH2F("B_VS_Energy_targ",";b, fm;E_{event} (target), GeV",200,0,20,250,0,300);
    TH2F* hImpactParameter_vs_Energy  =       new TH2F("B_VS_Energy_proj",";b, fm;E_{event} (projectile), GeV",200,0,20,250,0,36000);
    ...
    Double_t fEnergy=0;
    Double_t AverNeut=0, AverProt=0, AverDeut=0, NeventsNA49;
    Double_t px_beam, py_beam, pz_beam, E_beam;
    if (P_beam == 3) px_beam=0., py_beam=0., pz_beam=3.3600000, E_beam=3.430721;
	...
	for (long i = 0; i < lNEvents; i++)
	{
        ...
        fChain->GetEntry(i);
        if (fEvent->GetB()>2.0 && fEvent->GetB()<2.2) NeventsNA49++;
        fEnergy=0;
        ...
        for (int j=0;j<fNpa;j++)
        {
            ...
            if (fParticle->GetPdg()>1e9) 
            {
                ...
                if (fEvent->GetB()>2.0 && fEvent->GetB()<2.2 && fParticle->GetPdg()/10000%1000 == 1 && fParticle->GetPdg()/10%1000 == 2) AverDeut++;
                if (fParticle->GetPdg()/10000%1000 > 2) Zb2+=fParticle->GetPdg()/10000%1000;
                if (fParticle->GetPdg()/10000%1000 < 83 && fParticle->GetPdg()/10%1000 < 209) fNfrag+=1;
                if (fParticle->GetPdg()/10000%1000 < 83 && fParticle->GetPdg()/10%1000 < 209) fNspect+=1;
            ...
            else if (fParticle->GetPdg()==2212) 
            {
                ...
                hPT_vs_A                                -> Fill (pow(fMomentum.Px()*fMomentum.Px() + fMomentum.Py()*fMomentum.Py(),0.5), 1);
                ...
                if (fEvent->GetB()>2.0 && fEvent->GetB()<2.2) AverProt++;
                sumZ+=1;
                fNnucl +=1;
                fNspect+=1;
            }
            else if (fParticle->GetPdg()==2112) 
            {
                ...
                if (fEvent->GetB()>2.0 && fEvent->GetB()<2.2) AverNeut++;
                fNnucl+=1;
                fNspect+=1;
            }
            }
    ...
    std::cout << "N_n = " << AverNeut << "; N_p = " << AverProt << "; N_d = " << AverDeut << "; N_events = " << NeventsNA49 << endl;
    TCanvas* canvas = new TCanvas("canvas");
    gPad->SetTickx();
    gPad->SetTicky();
    hImpactParameter           ->Draw("COLZ");
    canvas->Print("../output/histos/QA_dcmqgsm_sps.pdf(");
