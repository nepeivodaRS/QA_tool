...
class UEvent : public TObject {
 ...
 private:
  ...
  Float_t      fEcc[10];      // Eccentricity array
 ...
 public:
  ...
  inline const Float_t* GetEcc() const {return fEcc;}
  ...
  void SetParameters(Int_t eventNr, Double_t b, Double_t phi, Int_t nes,
		     Int_t stepNr, Double_t stepT, Float_t* ecc);
  ...
  inline void SetEcc    (Float_t* newEcc) {for (int i = 0; i < 10; i++){fEcc[i] = newEcc[i];}}
